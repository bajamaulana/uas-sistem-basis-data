...
if ($customer_id) {
    // Total spent
    $stmt = $conn->prepare("SELECT SUM(total_amount) as total FROM orders WHERE customer_id = ? AND status = 'Completed'");
    ...
    $stmt = $conn->prepare("
        SELECT o.id, o.order_date, o.total_amount, o.status, 
               GROUP_CONCAT(CONCAT(p.product_name, ' (x', od.quantity, ')') SEPARATOR ', ') as items
        FROM orders o
...
<title>Transaction History | Ngopidea Artisanal Cafe</title>
<!-- Google Fonts -->
...
<a class="rounded-lg flex items-center gap-3 px-4 py-3 transition-all font-label-md text-label-md bg-primary-container text-on-primary-container font-bold" href="history.php">
...
<div class="mt-4">
<p class="text-headline-sm font-headline-sm">$<?= number_format($total_spent, 2) ?></p>
<p class="text-on-surface-variant font-label-md text-[12px]">Total Spent</p>
</div>
</div>
</div>
</div>
...
<input class="w-full pl-12 pr-4 py-3 bg-white border border-outline-variant/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-body-md" placeholder="Search by Order ID or items..." type="text">
</div>
...
<th class="pb-2 px-4">Order ID</th>
<th class="pb-2 px-4">Date</th>
<th class="pb-2 px-4">Items Purchased</th>
...
<tr><td colspan="5" class="py-4 px-4 text-center text-on-surface-variant">No transaction history found.</td></tr>
<?php else: ?>
<?php foreach ($transactions as $index => $tx): ?>
<tr class="order-row bg-white rounded-2xl shadow-sm border border-outline-variant/10 hover:shadow-md transition-shadow" data-status="<?= $tx['status'] ?>">
...
<td class="py-4 px-4 font-body-md text-on-surface"><?= htmlspecialchars($tx['items'] ?? '') ?></td>
